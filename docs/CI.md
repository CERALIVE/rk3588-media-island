...
| `self-tests` | every board/DT harness and every CI tool, including MPP hardening, module metadata, trace schemas, counter roots, the dual-core MPP parser, the RGA lifecycle parser, and `check-fault-seam-parity.sh`, passes scored fixtures. **A separate job on purpose** — see §3 |
...
| `cross-compile-rewrite` | the comparison candidate builds in its own disposable checkout **with the fault seam ON** (`CONFIG_ROCKCHIP_MPP_REWRITE_FAULT_INJECTION=y`, the overlay staged and digest-checked): both rewrite modules link with `-Werror`, KASAN and lockdep are required, competing MPP/`multi_rga`/mainline-RGA selections are rejected, pinned sparse runs with findings fatal, and both modules must publish OF aliases. Compile-only — no release, bootable slot or board verdict |
| `kunit-rewrite-fault` | the nine-control overlay's `rk-mpp-rewrite-fault` suite — the seam suite by name, not the upstream full suite — executes under arm64 QEMU/lockdep with `KCFLAGS=-Werror`; the raw log must be diagnostic-free and every registered case plus the production parity pairs must be `ok`; required by `ci-summary`. The adopted base's ten pre-existing `rk_mpp_rewrite` failures stay recorded as **NOT-OURS baseline debt** and are never claimed green |
