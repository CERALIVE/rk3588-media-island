...
sweep on a Rock 5B+ and an Orange Pi 5+ at island `v2026.9.2`, and the rewrite
comparison on the OPi test slot. The idle-window row was attempted on the Rock
...
| fault-seam parity | CI, no hardware | `scripts/check-fault-seam-parity.sh` — the comparison overlay's debugfs and harness names match the production seam's, and with `--ktap-log` every arm64-QEMU seam case has its production case-name counterpart |
| `tests/board/fault-matrix.sh` | a real Rock 5B+ or Orange Pi 5+ | the sixteen fault rows recover the device, consume the armed one-shot exactly once, and leave a healthy session's throughput intact — on either seam, via `--driver island\|rewrite` |
