...
| Reproduce Rock / OPi benchmark cells | `tests/board/bench-matrix.sh`, its single YAML cell list, and `bench-cell.c`; immutable DMA-BUF synthetic controls plus OPi HDMI capture, incremental results, and host scoring. Latency collects 180 timeoverlay frames, then `bench-latency.mjs` verifies every host-decoded timestamp; both timing endpoints are board-monotonic capture-pad→parsed-AU, not sensor/display. `make -C tests/board benchmark-selftest` tests the harness, not hardware. |
