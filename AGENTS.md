...
| Reproduce Rock / OPi benchmark cells | `tests/board/bench-matrix.sh`, its single YAML cell list, and `bench-cell.c`; immutable DMA-BUF synthetic controls plus OPi HDMI capture, incremental results, and host scoring. `make -C tests/board benchmark-selftest` tests the harness, not hardware. |
