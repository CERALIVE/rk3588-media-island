...
| Reproduce Rock benchmark cells | `tests/board/bench-matrix.sh`, its single YAML cell list, and `bench-cell.c`; immutable DMA-BUF source controls, incremental results, and host scoring. `make -C tests/board benchmark-selftest` tests the harness, not hardware. |
