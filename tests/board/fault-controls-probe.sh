...
# T3(ii), and it means a nine-way parity claim rests on five controls nobody has
...
#
# Usage:
#   fault-controls-probe.sh [--driver island|rewrite|auto] [--out DIR]
...
declare -rA SEAM_SYMBOL=(
	...
	[rewrite]=CONFIG_ROCKCHIP_MPP_REWRITE_FAULT_INJECTION
)
...
usage() {
	printf 'usage: %s [--driver island|rewrite|auto] [--out DIR] [--row NAME|all]\n' "$0" >&2
...
# `gap` line instead of aborting the snapshot with 77. The rewrite profile does
# not necessarily expose every counter the island does, and the contract's T4
...
# The island probe asserts the island client inventory; the rewrite exposes the
# same UAPI but is not the island, so it is asked only to answer at all.
board_capability() {
...
# bus that owns an encoder core and still publishes both attributes. The rewrite
# profile has no recorded name, which is exactly why the scan exists.
resolve_drv_dir() {
	local want dir
...
resolve_driver() {
	...
	for profile in island rewrite; do
		if "$CONFIG_FN" "${SEAM_SYMBOL[$profile]}"; then
			DRIVER="$profile"
			return 0
		fi
	done
...
# On the rewrite a missing node is GAP:not-implemented for that row alone.
seam_inventory() {
	...
	GAPS=$(IFS=,; printf '%s' "${missing[*]}")
	printf 'preflight verdict=GAP:not-implemented rows=%s driver=%s\n' "$GAPS" "$DRIVER"
	return 0
}
...
idle_snapshot() {
	...
	if [[ $DRIVER == rewrite ]]; then
		file=/sys/kernel/debug/rk_mpp_rewrite
		[[ -r $file/state && -r $file/queued_job_count ]] || return "$GATED"
		awk '/^# hardware:/{hw=1;next} /^# /{hw=0}
			hw && NF==14{printf "%s busy %d\n",$1,($11!=0)}' "$file/state" >"$dest"
		printf 'global queue_depth %s\n' "$(<"$file/queued_job_count")" >>"$dest"
		for file in "$FAULT_DEBUG"/*consumed; do
			[[ ! -r $file ]] || printf 'fault %s %s\n' "${file##*/}" "$(<"$file")" >>"$dest"
		done
	else
...
row_idle_iommu_fault() (
	...
	*) printf 'row=idle-iommu-fault verdict=FAIL reason=not-fired state=%s\n' "$state"; return "$FAIL" ;;
	...
	for metric in busy queue_depth dmabufs iommu_maps; do
		if [[ $DRIVER == rewrite && $metric == iommu_maps ]]; then
			gaps=' gaps=iommu_maps=GAP:no-sessions-summary'
			continue
		fi
...
run_row() {
	local row=$1
	if row_is_gap "$row"; then
		printf 'row=%s verdict=GAP:not-implemented driver=%s\n' "$row" "$DRIVER"
		return "$GATED"
	fi
...
fx_config_has() {
	...
	"${SEAM_SYMBOL[island]}"|"${SEAM_SYMBOL[rewrite]}") return 0 ;;
	esac
	return 1
}
...
idle_self_test() {
	...
	for driver in island rewrite; do
	...
	printf 'VERDICT: PASS (idle-only row: both PM readings, both drivers, negative assertions and admission gates; synthetic only)\n'
}
...
self_test() {
	...
	# The rewrite profile labels a missing control instead of failing on it.
	st_reset session-alloc
	DRIVER=rewrite
	rm -f "$FAULT_DEBUG/${KNOB[session-alloc]}"
	st_leg rewrite-gap-not-implemented 77 'verdict=GAP:not-implemented'
	...
	leg=rewrite-gap-not-implemented want=77 got=77 token=found verdict=PASS
	GOLDEN
	)
	...
	((rc == 0)) || return "$rc"
	printf 'VERDICT: PASS (5 rows proven fire-once, 13 RED legs proven red, 4 gates proven real)\n'
	idle_self_test
}
...
main() {
	...
	[[ $DRIVER == island || $DRIVER == rewrite || $DRIVER == auto ]] || { usage; return "$USAGE"; }
