...
static int self_test(void)
{
	...
	if (!hw_support_matches(ISLAND_HW_SUPPORT, ISLAND_HW_SUPPORT) ||
	    ...
	    hw_support_matches(ISLAND_HW_SUPPORT ^ 1u, ISLAND_HW_SUPPORT)) {
		fprintf(stderr, "FAIL: hw_support comparison is not discriminating\n");
		return EXIT_FAIL;
	}
	printf("expect_bits.island=0x%08x\n", ISLAND_HW_SUPPORT);

	if (!parse_hw_mask("0x00012200", &parsed) || parsed != ISLAND_HW_SUPPORT ||
	    !parse_hw_mask("74240", &parsed) || parsed != ISLAND_HW_SUPPORT) {
...
static void usage(const char *argv0)
{
	fprintf(stderr,
		...
		"  --expect-bits MASK  require exactly MASK (hexadecimal or decimal)\n"
