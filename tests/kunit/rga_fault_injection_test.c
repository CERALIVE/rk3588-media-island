...
static void rga_fault_timeout_recovery_test(struct kunit *test)
{
	...
	for (i = 0; i < 2; i++) {
		...
		f->job.timestamp.hw_execute = ktime_sub_ms(ktime_get(), 1001);
...
{
	struct rga_fault_fixture *f = test->priv;
	struct media_recovery *recovery = &f->scheduler.recovery;
	s64 previous_epoch = atomic64_read(&recovery->recovery_epoch);

	/* A previous job already recovered before this START raises an immediate IRQ. */
	atomic64_set(&recovery->claimed_epoch, previous_epoch);
	f->reset_at_start = true;
	KUNIT_ASSERT_EQ(test, rga_job_run(&f->job, &f->scheduler), 0);
	KUNIT_EXPECT_EQ(test, f->resets, 2);
	KUNIT_EXPECT_EQ(test, atomic64_read(&recovery->claimed_epoch), previous_epoch + 1);
}
