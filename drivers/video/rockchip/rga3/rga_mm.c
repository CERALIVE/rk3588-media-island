...
static struct page *rga_virt_original_page(struct rga_virt_addr *virt, int index)
{
	struct rga_shadow_node *shadow;

	list_for_each_entry(shadow, &virt->shadow_list, node)
		if (shadow->page_idx == index)
			return shadow->orig_page;
	return virt->pages[index];
}

static bool rga_virt_under_4g(struct rga_virt_addr *virt)
{
	int i;

	for (i = 0; i < virt->page_count; i++)
		if (page_to_phys(rga_virt_original_page(virt, i)) >= SZ_4G)
			return false;
	return true;
}
...
static unsigned int rga_dma_max_segment_size(struct device *dev)
{
	...
	max_segment = min_t(size_t, max_segment, dma_get_max_seg_size(dev));
	return round_down(max_segment, PAGE_SIZE);
}
...
static int rga_mm_check_range_sgt(struct sg_table *sgt)
{
	...
	for_each_sg(sgt->sgl, sg, sgt->orig_nents, i) {
		s_phys = sg_phys(sg);
		if (s_phys >= SZ_4G || sg->length > SZ_4G - s_phys)
			return 0;
	}
				 ...
				 struct rga_job *job)
{
	...
	struct rga_dma_buffer *buffer;
	struct device *map_dev;
	struct rga_scheduler_t *map_scheduler;
	struct rga_scheduler_t *scheduler;
	...
	if (scheduler->data->mmu == RGA_MMU) {
		map_scheduler = rga_drvdata->scheduler[rga_drvdata->map_scheduler_index];
		if (!map_scheduler || !map_scheduler->iommu_info) {
			ret = -ENODEV;
			goto free_buffer;
		}
	}
	/* The exporter owns its SG list: inspect it through a capable attachment. */
	map_dev = map_scheduler->iommu_info ?
		map_scheduler->iommu_info->default_dev : map_scheduler->dev;
	ret = rga_mm_map_external_dma_buffer(external_buffer, buffer,
					     map_scheduler, map_dev);
	if (ret < 0) {
	...
	if (map_scheduler->data->mmu == RGA_IOMMU)
	...
	internal_buffer->scheduler = map_scheduler;
				...
				struct rga_job *job, int write_flag)
{
	...
	if (rga_virt_under_4g(virt_addr))
		mm_flag |= RGA_MEM_UNDER_4G;
	else if (scheduler->data->mmu == RGA_MMU) {
		scheduler = rga_drvdata->scheduler[rga_drvdata->map_scheduler_index];
		if (!scheduler || !scheduler->iommu_info) {
			ret = -ENODEV;
			goto free_sgt;
		}
	}
			   ...
			   u64 *size)
{
	...
	if (!mm || !handle || handle > U32_MAX || !size)
		return -EINVAL;

	mutex_lock(&mm->lock);
	buffer = rga_mm_lookup_handle(mm, handle);
	if (!buffer)
		ret = -EINVAL;
	else if (!rga_mm_handle_authorized(buffer, session))
		ret = -EPERM;
	else if (!buffer->size)
		ret = -EINVAL;
	else
...
static bool rga_mm_channel_needs_mmu(struct rga_job *job, struct rga_job_buffer *buffer)
{
	return rga_mm_is_need_mmu(job, buffer->y_addr) ||
	       rga_mm_is_need_mmu(job, buffer->uv_addr) ||
	       rga_mm_is_need_mmu(job, buffer->v_addr);
}
			       ...
			       struct rga_req *req)
{
	...
	src_mmu_en = rga_mm_channel_needs_mmu(job, &buffers->src_buffer);
	src1_mmu_en = rga_mm_channel_needs_mmu(job, &buffers->src1_buffer);
	dst_mmu_en = rga_mm_channel_needs_mmu(job, &buffers->dst_buffer);
	els_mmu_en = rga_mm_channel_needs_mmu(job, &buffers->els_buffer);
...
rga2_stage_get(struct rga_job *job, struct rga_internal_buffer *buffer)
{
	...
	struct dma_buf *origin;
	size_t staged_bytes = job->rga2_user_bytes;
	unsigned int i;
	int ret;
				   ...
				   enum dma_data_direction dir)
{
	...
	for (i = 0; i < job_buf->rga2_user_sgt_count; i++)
		rga_free_sgt(&job_buf->rga2_user_sgt[i]);
	job_buf->rga2_user_sgt_count = 0;
					    ...
					    int32_t *use_dma_address)
{
	...
	*use_dma_address = false;
	...
	if (buffer->type == RGA_VIRTUAL_ADDRESS &&
		...
		dma_sync_sg_for_cpu(buffer->dma_buffer->map_dev,
			buffer->dma_buffer->sgt->sgl,
			buffer->dma_buffer->sgt->orig_nents, DMA_BIDIRECTIONAL);
		*use_dma_address = true;
		return rga2_user_stage_sgt(job, job_buf, buffer);
	}
...
static void rga_mm_free_page_table(struct rga_job_buffer *job_buf)
{
	if (job_buf->page_table_mapped)
		dma_unmap_single(job_buf->page_table_dev, job_buf->page_table_dma,
				 job_buf->page_count * sizeof(*job_buf->page_table),
				 DMA_TO_DEVICE);
	...
	job_buf->page_table_dev = NULL;
	job_buf->page_table_dma = 0;
	job_buf->page_count = 0;
	job_buf->order = 0;
}
			       ...
			       enum dma_data_direction dir)
{
	...
	/* Queued tasks retain distinct tables until their terminal cleanup. */
	job_buf->page_table_dma = dma_map_single(job->scheduler->dev, page_table,
				page_count * sizeof(*page_table), DMA_TO_DEVICE);
...
err_free_page_table:
	...
	rga_mm_free_page_table(job_buf);
	return ret;
}

static struct rga_dma_buffer *
rga_mm_find_job_iommu_mapping(struct rga_job_buffer *job_buf,
			      struct rga_internal_buffer *origin)
{
	int i;
	...
	for (i = 0; i < job_buf->iommu_mapping_count; i++)
		if (job_buf->iommu_mapping[i].origin == origin)
			return job_buf->iommu_mapping[i].mapping;

	return NULL;
}

static void rga_mm_unmap_job_iommu_mapping(struct rga_job_buffer *job_buf,
					   int index)
{
	struct rga_dma_buffer *mapping = job_buf->iommu_mapping[index].mapping;
	struct sg_table *owned_sgt = job_buf->iommu_mapping[index].owned_sgt;
	int last = --job_buf->iommu_mapping_count;

	if (mapping->attach)
		rga_dma_unmap_buf(mapping);
	else
		rga_dma_unmap_sgt(mapping);
	if (owned_sgt)
		rga_free_sgt(&owned_sgt);
	kfree(mapping);

	if (index != last)
		job_buf->iommu_mapping[index] = job_buf->iommu_mapping[last];
	memset(&job_buf->iommu_mapping[last], 0,
	       sizeof(job_buf->iommu_mapping[last]));
}

static void rga_mm_release_job_iommu_mappings(struct rga_job_buffer *job_buf)
{
	while (job_buf->iommu_mapping_count)
		rga_mm_unmap_job_iommu_mapping(job_buf,
		      ...
		      struct rga_internal_buffer *origin)
{
	...
	mapping = rga_mm_find_job_iommu_mapping(job_buf, origin);
	return mapping ? mapping : origin->dma_buffer;
}

static struct sg_table *
rga_mm_alloc_job_virt_sgt(struct rga_internal_buffer *origin,
			  size_t *real_offset)
{
	struct rga_virt_addr *virt_addr = origin->virt_addr;
	size_t map_offset;
	size_t map_size;

	if (!virt_addr || !virt_addr->pages || virt_addr->page_count <= 0)
		return ERR_PTR(-EINVAL);
	...
	return rga_alloc_sgt(virt_addr->pages, virt_addr->page_count,
			     map_offset, map_size, GFP_KERNEL);
}

static struct sg_table *
rga_mm_alloc_job_phys_sgt(struct rga_internal_buffer *origin)
{
	phys_addr_t aligned = ALIGN_DOWN(origin->phys_addr, PAGE_SIZE);
	size_t offset = offset_in_page(origin->phys_addr);
	struct page **pages;
	struct sg_table *sgt;
	size_t span;
	u32 page_count;
	...
	if (!origin->size || check_add_overflow(offset, origin->size, &span))
		return ERR_PTR(-EINVAL);
	page_count = DIV_ROUND_UP(span, PAGE_SIZE);
	if (!page_count)
		return ERR_PTR(-EINVAL);
	...
	ret = rga_get_phys_addr_pages(pages, aligned, page_count);
	if (ret) {
		sgt = ERR_PTR(ret);
		goto out_free_pages;
	}

	sgt = rga_alloc_sgt(pages, page_count, offset, origin->size, GFP_KERNEL);
...
static struct rga_dma_buffer *
rga_mm_map_job_iommu_buffer(struct rga_job *job,
			    struct rga_job_buffer *job_buf,
			    struct rga_internal_buffer *origin)
{
	struct rga_dma_buffer *mapping;
	struct sg_table *owned_sgt = NULL;
	size_t real_offset = 0;
	int ret;
	int slot;
	...
	if (rga_mm_is_invalid_dma_buffer(origin->dma_buffer))
		return ERR_PTR(-EINVAL);
	if (origin->dma_buffer->map_dev == job->scheduler->dev)
		return origin->dma_buffer;

	mapping = rga_mm_find_job_iommu_mapping(job_buf, origin);
	if (mapping)
		return mapping;
	if (job_buf->iommu_mapping_count >= ARRAY_SIZE(job_buf->iommu_mapping))
	...
	case RGA_VIRTUAL_ADDRESS:
		...
		ret = rga_dma_map_sgt(owned_sgt, mapping, DMA_BIDIRECTIONAL,
				      job->scheduler->dev);
		if (!ret)
			mapping->offset = real_offset;
		break;
	case RGA_PHYSICAL_ADDRESS:
		owned_sgt = rga_mm_alloc_job_phys_sgt(origin);
		if (IS_ERR(owned_sgt)) {
			ret = PTR_ERR(owned_sgt);
			owned_sgt = NULL;
			break;
	...
	mapping->iova = mapping->dma_addr;
	slot = job_buf->iommu_mapping_count++;
	job_buf->iommu_mapping[slot].origin = origin;
	job_buf->iommu_mapping[slot].mapping = mapping;
	job_buf->iommu_mapping[slot].owned_sgt = owned_sgt;
					 ...
					 enum dma_data_direction dir)
{
	...
	scheduler = dma_buffer == buffer->dma_buffer ? buffer->scheduler :
		    job->scheduler;
	if (scheduler == NULL) {
	...
	if ((buffer->mm_flag & RGA_MEM_PHYSICAL_CONTIGUOUS) &&
		...
		if (scheduler->data->mmu == RGA_IOMMU) {
			...
			if (dma_mapping_error(dma_buffer->map_dev, iova)) {
				rga_job_err(job, "invalid iova for dma-buffer with IOMMU device!\n");
				return -EFAULT;
			}

			dma_sync_single_for_device(dma_buffer->map_dev,
				      ...
				      enum dma_data_direction dir)
{
	...
	if (job->scheduler && job->scheduler->data->mmu == RGA_MMU &&
	    buffer->type == RGA_VIRTUAL_ADDRESS &&
	    (job->flags & RGA_JOB_RGA2_STAGEABLE_DMA_BUF)) {
		dma_sync_sg_for_device(buffer->dma_buffer->map_dev,
			buffer->dma_buffer->sgt->sgl,
			buffer->dma_buffer->sgt->orig_nents, DMA_TO_DEVICE);
		return 0;
	}
	...
	if ((buffer->mm_flag & RGA_MEM_PHYSICAL_CONTIGUOUS) &&
		...
		if (scheduler->data->mmu == RGA_IOMMU) {
			...
			dma_sync_single_for_cpu(dma_buffer->map_dev,
				iova, buffer->size, dir);
		} else {
				  ...
				  uint64_t *channel_addr)
{
	...
	case RGA_IOMMU:
		dma_buffer = rga_mm_map_job_iommu_buffer(job, job_buf,
						     internal_buffer);
		...
		addr = dma_buffer->iova + dma_buffer->offset;
		if (dma_mapping_error(dma_buffer->map_dev, (dma_addr_t)addr)) {
			rga_job_err(job, "invalid iova for dma-buffer with IOMMU device!\n");
			return -EFAULT;
		}
			     ...
			     enum dma_data_direction dir)
{
	...
	*buf = rga_mm_lookup_handle(mm, handle);
	...
	ret = rga_mm_get_buffer_info(job, internal_buffer, job_buf,
				     channel_addr);
	if (ret < 0) {
	...
	if (internal_buffer->mm_flag & RGA_MEM_FORCE_FLUSH_CACHE) {
		 ...
		 * to flush the cache when the virtual address is used.
		 */
		ret = rga_mm_sync_dma_sg_for_device(
			internal_buffer,
			rga_mm_job_dma_buffer(job_buf, internal_buffer), job, dir);
		if (ret < 0) {
			      ...
			      enum dma_data_direction dir)
{
	if (internal_buffer->mm_flag & RGA_MEM_FORCE_FLUSH_CACHE && dir != DMA_NONE)
		if (rga_mm_sync_dma_sg_for_cpu(
				internal_buffer,
				rga_mm_job_dma_buffer(job_buf, internal_buffer), job,
				dir))
					   ...
					   enum dma_data_direction dir)
{
	...
	if (job_buf->y_addr) {
		rga_mm_put_buffer(mm, job, job_buf, job_buf->y_addr, dir);
		job_buf->y_addr = NULL;
	}
	if (job_buf->uv_addr) {
		rga_mm_put_buffer(mm, job, job_buf, job_buf->uv_addr, dir);
		job_buf->uv_addr = NULL;
	}
	if (job_buf->v_addr) {
		rga_mm_put_buffer(mm, job, job_buf, job_buf->v_addr, dir);
		job_buf->v_addr = NULL;
	}
					  ...
					  enum dma_data_direction dir)
{
	...
	if (img->uv_addr > 0) {
		...
		if (handle > 0) {
			ret = rga_mm_get_buffer(mm, job, job_buf, handle, &img->yrgb_addr,
		...
		handle = img->uv_addr;
		if (handle > 0) {
			ret = rga_mm_get_buffer(mm, job, job_buf, handle, &img->uv_addr,
		...
		handle = img->v_addr;
		if (handle > 0) {
			ret = rga_mm_get_buffer(mm, job, job_buf, handle, &img->v_addr,
	...
	if (job->scheduler->data->mmu == RGA_MMU &&
	    rga_mm_channel_needs_mmu(job, job_buf)) {
				  ...
				  struct rga_req *req)
{
	...
	buffers->dst_buffer.writable = true;
					    ...
					    enum dma_data_direction dir)
{
	...
	if (job_buffer->ready &&
	    job_buffer->addr->mm_flag & RGA_MEM_FORCE_FLUSH_CACHE && dir != DMA_NONE)
		if (rga_mm_sync_dma_sg_for_cpu(
				job_buffer->addr,
				rga_mm_job_dma_buffer(job_buffer, job_buffer->addr),
				job, dir))
			rga_job_err(job, "sync sgt for cpu error!\n");
	...
	rga_mm_release_job_iommu_mappings(job_buffer);
	rga_mm_unmap_buffer(job_buffer->addr);
	kfree(job_buffer->addr);
	job_buffer->addr = NULL;
	job_buffer->ready = false;
	rga_mm_free_page_table(job_buffer);
}
					 ...
					 int write_flag)
{
	...
	ret = rga_mm_get_buffer_info(job, buffer, job_buffer,
				     &img->yrgb_addr);
	if (ret < 0) {
	...
	if (buffer->mm_flag & RGA_MEM_FORCE_FLUSH_CACHE) {
		ret = rga_mm_sync_dma_sg_for_device(
			buffer, rga_mm_job_dma_buffer(job_buffer, buffer), job, dir);
		if (ret < 0) {
			rga_job_err(job, "sync sgt for device error!\n");
			goto error_unmap_buffer;
		}
...
static int rga_mm_prepare_channel(struct rga_job *job,
				  struct rga_job_buffer *channel, bool writable)
{
	struct rga_internal_buffer *buffer;
	int ret;
	...
	if (!buffer)
		return -ENOMEM;
	ret = rga_mm_map_buffer(channel->ex_addr, buffer, job, writable);
	if (ret) {
		kfree(buffer);
		return ret;
	}
...
int rga_mm_prepare_job_info(struct rga_job *job)
{
	...
	job->task_buffers = kvcalloc(job->task_count, sizeof(*job->task_buffers), GFP_KERNEL);
	if (!job->task_buffers)
		return -ENOMEM;
	if (job->flags & RGA_JOB_DEBUG_FAKE_BUFFER)
		return 0;
	...
	if (!map_scheduler)
		return -ENODEV;
	ret = rga_power_enable(map_scheduler);
	if (ret)
		return ret;
	job->scheduler = map_scheduler;
	for (i = 0; i < job->task_count; i++) {
		req = &job->task_list[i];
		buffers = &job->task_buffers[i];
		if (req->handle_flag & 1)
			continue;
		ret = rga_mm_get_external_buffer(job, buffers, req);
		if (ret)
			break;
		ret = rga_mm_prepare_channel(job, &buffers->src_buffer, false);
		if (ret)
			break;
		ret = rga_mm_prepare_channel(job, &buffers->dst_buffer, true);
		if (ret)
			break;
		ret = rga_mm_prepare_channel(job, &buffers->src1_buffer, false);
		if (ret)
			break;
		ret = rga_mm_prepare_channel(job, &buffers->els_buffer, false);
		if (ret)
			break;
	}
	if (ret) {
	...
	rga_power_disable(map_scheduler);
	return ret;
}
...
int rga_mm_map_job_info(struct rga_job *job)
{
	...
	if (job->scheduler->data->mmu == RGA_IOMMU) {
		ret = rga_iommu_flush_tlb(job->scheduler->iommu_info);
		if (ret)
			goto err_unmap_job_info;
	}
